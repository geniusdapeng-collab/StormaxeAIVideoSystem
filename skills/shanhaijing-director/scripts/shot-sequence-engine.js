#!/usr/bin/env node
/**
 * Shot Sequence & Impact Engine v2.0-Peng
 * 镜头序列冲击引擎 — 时长×运镜×转场 = 视觉冲击力
 * 
 * 🆕 v2.0-Peng (2026-06-11): LLM推理接入
 *   - LLM分析相邻镜头组合冲击力、生成转场建议、检测问题序列
...
class ShotSequenceEngine {
  ...
  async analyzeSequence(shots) {
    ...
    // 🆕 v2.0-Peng: 尝试LLM推理
    try {
      const llmResult = await this._analyzeWithLLM(shots);
      if (llmResult) {
        console.log('   ✅ LLM镜头序列分析完成');
        return llmResult;
      }
    } catch (err) {
      console.log(`   ⚠️ LLM序列分析失败: ${err.message?.substring(0, 60)}，降级本地规则`);
    }

    return this._analyzeLocal(shots);
  }

  /**
   * 🆕 v2.0-Peng: LLM镜头序列分析
   */
  async _analyzeWithLLM(shots) {
    const llm = new LLMReasoningLayer();
    const shotSummary = shots.map((s, i) => ({
      index: i, id: s.id, type: s.type, scale: s.scale,
      camera: s.camera, duration: s.duration, emotion: s.emotion
    }));

    const result = await llm.llmReason({
      stage: 'shot-sequence-analysis',
      systemPrompt: 'You are a cinematography expert specializing in shot sequencing and visual impact analysis. Analyze adjacent shot pairs for scale jumps, movement energy contrast, duration contrast, and transition quality. Detect problem sequences (repetitive shots, unmotivated jump cuts, rhythm breaks). Return a JSON object with: sequences (array of pair analyses with impactScore 1-10, transitionSuggestion, scaleJump, energyContrast), impactCurve (array of scores), warnings (array of problem descriptions), rhythmAnalysis (overall pacing assessment), avgImpact, maxImpact, minImpact, strongImpactPoints.',
      userPrompt: `Analyze this shot sequence for visual impact:\n${JSON.stringify(shotSummary, null, 2)}`,
      level: 'medium',
      fallback: () => null
    });
 ...
 * 快速分析镜头序列
 */
async function analyzeShotSequence(shots, options = {}) {
 ...
 * 生成可视化报告
 */
async function generateSequenceReport(shots, options = {}) {
  const engine = new ShotSequenceEngine(options);
