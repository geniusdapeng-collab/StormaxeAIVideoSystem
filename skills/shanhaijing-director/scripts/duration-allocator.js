#!/usr/bin/env node
/**
 * Duration Allocator v2.0-Peng
 * 智能镜头时长分配器 — 节奏感的灵魂
 * 
 * 🆕 v2.0-Peng (2026-06-11): LLM叙事节奏智能分配接入
 *   - LLM作为主分配引擎，根据叙事重要性智能分配时长
 *   - 本地五层加权降级为fallback
 * 
...
/**
   ...
   * @returns {Object} 分配结果
   */
  async allocate(shots, targetDuration) {
    if (!shots || shots.length === 0) {
      return { shots: [], totalDuration: 0, deviation: 0, passed: false };
    }

    // v2.0-Peng: Try LLM narrative rhythm allocation
    try {
      const llmResult = await this._allocateWithLLM(shots, targetDuration);
      if (llmResult) {
        if (this.debug) console.log('  [DurationAllocator] LLM narrative rhythm allocation complete');
        return llmResult;
      }
    } catch (err) {
      if (this.debug) console.log('  [DurationAllocator] LLM allocation failed, falling back to local rules');
    }

    // Fallback: local five-layer weighting
    return this._allocateLocal(shots, targetDuration);
  }

  async _allocateWithLLM(shots, targetDuration) {
    const { LLMReasoningLayer } = require('./llm-reasoning-layer');
    const llmLayer = new LLMReasoningLayer();

    const shotSummaries = shots.map((s, i) => {
      const desc = (s.description || s.narration || '').substring(0, 80);
      return 'S' + String(i).padStart(2,'0') + ' | type:' + (s.type||'normal') + ' | tension:' + (s.tension||50) + ' | camera:' + (s.camera||'N/A') + ' | desc:"' + desc + '"';
    }).join('\n');

    const systemPrompt = 'You are a film editor specializing in narrative rhythm. Assign optimal durations to shots based on narrative importance, emotional weight, and visual complexity.\n\nPRINCIPLES: Climax/reveal=8-15s, Establishing=3-6s, Action=5-10s, Emotional=6-12s, Transition=2-5s. Total must match target.\n\nOUTPUT: Valid JSON only: {"allocations":[{"id":"S00","duration":8,"reason":"brief reason"}],"totalDuration":60,"rhythmNotes":"Chinese rhythm analysis"}';

    const userPrompt = 'Target: ' + targetDuration + 's. ' + shots.length + ' shots:\n' + shotSummaries + '\n\nAssign durations. Total = ' + targetDuration + 's.';

    const result = await llmLayer.llmReason({
      stage: 'duration-allocation',
      systemPrompt, userPrompt,
      level: 'light',
      llmOptions: { temperature: 0.3, maxTokens: 4000 },
      fallback: () => null
    });

    if (!result.success || result.fallbackUsed || !result.result) return null;
    const jsonStr = LLMReasoningLayer.extractJSON(result.result);
    if (!jsonStr) return null;
    const llmAlloc = JSON.parse(jsonStr);
    if (!llmAlloc.allocations || !Array.isArray(llmAlloc.allocations)) return null;

    const allocMap = {};
    for (const a of llmAlloc.allocations) allocMap[a.id] = { duration: Math.round(a.duration), reason: a.reason };

    const profile = this.rhythmProfile;
    let allocatedTotal = 0;
    for (let i = 0; i < shots.length; i++) {
      const shotId = shots[i].id || 'S' + String(i).padStart(2, '0');
      const alloc = allocMap[shotId];
      if (alloc) {
        shots[i].duration = Math.max(profile.minShot, Math.min(profile.maxShot, alloc.duration));
        shots[i]._durationReason = alloc.reason || 'LLM';
      } else {
        shots[i].duration = Math.round(targetDuration / shots.length);
        shots[i]._durationReason = 'LLM not covered, local fallback';
      }
      allocatedTotal += shots[i].duration;
    }

    const diff = targetDuration - allocatedTotal;
    if (Math.abs(diff) > 0 && shots.length > 0) {
      const sorted = shots.map((s, i) => ({ idx: i, dur: s.duration })).sort((a, b) => b.dur - a.dur);
      let remaining = diff;
      for (const { idx } of sorted) {
        if (remaining === 0) break;
        const adj = remaining > 0 ? 1 : -1;
        const nd = shots[idx].duration + adj;
        if (nd >= profile.minShot && nd <= profile.maxShot) { shots[idx].duration = nd; remaining -= adj; }
      }
    }

    const finalTotal = shots.reduce((sum, s) => sum + s.duration, 0);
    return {
      shots, totalDuration: finalTotal, targetDuration,
      deviation: Math.abs(finalTotal - targetDuration) / targetDuration,
      passed: Math.abs(finalTotal - targetDuration) / targetDuration <= 0.05,
      profile: this._getProfileName(),
      llmRhythmNotes: llmAlloc.rhythmNotes || '',
      allocations: shots.map((s, i) => ({ id: s.id || 'S' + String(i).padStart(2, '0'), type: s.type || 'normal', duration: s.duration, reason: s._durationReason || '' }))
    };
  }

  _allocateLocal(shots, targetDuration) {
 ...
 * @returns {Object} 分配结果
 */
async function allocateDurations(shots, targetDuration, videoType = 'action') {
  const allocator = new DurationAllocator({ videoType });
  return await allocator.allocate(shots, targetDuration);
}
