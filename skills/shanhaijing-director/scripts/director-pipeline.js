...
`;
  ...
  _summarizePRD(prd) {
    ...
    // 提取标题
    const titleMatch = prd.match(/#\s*产品需求文档.*?[-\-]\s*(.+)/);
...
// fix15-v7: guard
  ...
  _buildShotContext(shot, storyPlan) {
    ...
    lines.push(`Story Outline: ${storyPlan.outline?.substring(0, 200) || 'Not provided'}`);
...
function main() {
  ...
  if (command === 'run' || command === 'pre-production') {
    ...
    if (fs.existsSync(prdPath)) {
      ...
      const titleMatch = prdContent.match(/#\s*产品需求文档.*?[-\-]\s*(.+)/);
      if (titleMatch) {
        userInput.title = titleMatch[1].trim();
      }
      const outlineMatch = prdContent.match(/##\s*2\.\s*故事大纲([\s\S]*?)(?=##\s*\d+\.|$)/);
      if (outlineMatch) {
        userInput.outline = outlineMatch[1].trim().substring(0, 200);
      }
    }
...
神兽: ${beastName || '无'}
故事大纲: ${(storyPlan.outline || '').substring(0, 200)}
...
llmDesign = jsonStr ? JSON.parse(jsonStr) : null;
    ...
    // 生成开场标题
    const theme = storyPlan.description || storyPlan.outline || '';
