#!/usr/bin/env node
/**
 * Dialogue Engine — 台词/对白生成器 v2.0-Peng
 *
 * 🆕 v2.0-Peng (2026-06-11): LLM台词生成接入
 *   - LLM根据角色性格和场景生成更自然的台词
 *   - 本地模板降级为fallback
 * 
 * 🆕 v1.3-Peng-fix 更新（2026-05-31）：旁白禁止约束
 * - 删除旁白/解说（Voiceover）生成，只生成角色台词（Dialogue）
 * - 所有台词必须包含 SPEAKER + TYPE + EMOTION + TEXT + LIP_SYNC: YES
 * - 无角色镜头不生成旁白，改为环境声音（非语音）
 * - 音频类型只返回 'dialogue' 或 'battle_cry'，禁止 'narration'
 * 
 * 输入：story-plan.json
 ...
 *   2. 按镜头类型决定对白长度和情绪
 *   3. 角色口吻差异化（主角/配角）——旁白已删除，见AGENTS.md旁白禁止约束
 *   4. 输出Seedance可理解的"说话"提示词增强，所有台词必须 LIP_SYNC: YES
 * 
 * 用法：
...
const DIALOGUE_DENSITY = {
  ...
  educational: 2,  // 教育片：角色台词解说+偶尔角色对话（旁白禁止，改为角色台词）
  commercial: 2,   // 商业片：CTA文案+产品解说（旁白禁止，改为角色台词）
  documentary: 2,  // 纪录片：角色台词为主+采访对白（旁白禁止，改为角色台词）
...
const CHARACTER_VOICES = {
  ...
  // 旁白/解说 [已废弃：旁白禁止约束，所有音频必须是角色台词]
  narrator: {
    style: '已废弃——旁白禁止，见AGENTS.md旁白禁止约束',
    vocabulary: 'N/A',
    emotionRange: ['N/A'],
    patterns: ['N/A']
  }
...
// ============ 主生成函数 ============
async function generateDialogues(plan) {
  ...
  for (const shot of shots) {
    ...
    if (!speaker) {
      dialogues.push({
        ...
        lines: [],
        promptEnhancement: '',
        audioType: 'environmental' // 环境声音，非台词
      });
      continue;
    }
    
    // 生成对白内容
    const lines = await generateLines(shot, videoType, speaker, shot.duration);
...
async function generateMultiCharDialogue(shot, videoType, duration, pool) {
  const chars = shot.characters || [];
  if (chars.length < 2) return await generateLines(shot, videoType, { name: chars[0] || '角色' }, duration);
...
function generatePromptEnhancement(lines, speaker, shot) {
  ...
  // 动作片简短台词
  if (shot.duration <= 3) {
    return `，角色${speaker.name}口中喊出简短有力的台词（LIP_SYNC: YES，口型同步，非画面文字）`;
  }
  
  // 一般对话（长对白截断显示）
  const displayText = line.text.length > 20 ? line.text.substring(0, 20) + '...' : line.text;
  return `，角色${speaker.name}正在说话："${displayText}"（LIP_SYNC: YES，口型同步，非画面文字）`;
}
...
function formatDialogueDocument(result) {
  ...
  for (const d of result.dialogues) {
    ...
    for (const line of d.lines) {
      md += `- SPEAKER: ${line.speaker} | TYPE: ${line.type || '独白'} | EMOTION: ${line.emotion} | TEXT: "${line.text}" | LIP_SYNC: ${line.lipSync || 'YES'} | 预估时长: ${line.estimatedDuration.toFixed(1)}秒\n\n`;
    }
    
    md += `**提示词增强**: ${d.promptEnhancement}\n\n`;
    md += `**音频类型**: ${d.audioType}（禁止旁白，纯角色台词）\n\n`;
    md += `---\n\n`;
  }
