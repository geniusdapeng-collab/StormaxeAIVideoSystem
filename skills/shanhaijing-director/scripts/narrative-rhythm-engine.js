...
class NarrativeRhythmEngine {
   ...
   * 🆕 v2.0-Peng: LLM叙事节奏分析
   */
  async analyzeRhythm(shots, options = {}) {
    try {
      const llmResult = await this._analyzeWithLLM(shots, options);
      if (llmResult) {
        console.log('   ✅ LLM叙事节奏分析完成');
        return llmResult;
      }
    } catch (err) {
      console.log(`   ⚠️ LLM节奏分析失败: ${err.message?.substring(0, 60)}，降级本地分析`);
    }

    return this._analyzeLocal(shots, options);
  }

  async _analyzeWithLLM(shots, options = {}) {
    const llm = new LLMReasoningLayer();
    const shotSummary = (shots || []).map((s, i) => ({
      index: i, id: s.id, type: s.type, duration: s.duration,
      emotion: s.emotion, camera: s.camera
    }));

    const result = await llm.llmReason({
      stage: 'narrative-rhythm',
      systemPrompt: `You are a narrative rhythm expert for cinematic video production. Analyze the story beats and suggest pacing improvements for maximum emotional impact. Consider: three-act micro-narrative structure, emotion curve stages, beat design, tension build/release patterns. Return JSON: { rhythmType: "build"|"release"|"wave"|"collapse", emotionCurve: [{position, intensity, label}], pacingSuggestions: [strings], problemBeats: [strings], overallAssessment: string }.`,
      userPrompt: `Shots to analyze:\n${JSON.stringify(shotSummary, null, 2)}`,
      level: 'medium',
      fallback: () => null
    });
  ...
  _analyzeLocal(shots, options = {}) {
    const totalDuration = (shots || []).reduce((sum, s) => sum + (s.duration || 0), 0);
    const emotionCurve = this.EMOTION_CURVES.build.stages.map(s => ({
      position: s.pos, intensity: s.intensity, label: s.label
    }));
    ...
    return {
      ...
      pacingSuggestions: [],
      problemBeats: [],
      overallAssessment: 'Local rule-based rhythm analysis'
    };
  }
}
