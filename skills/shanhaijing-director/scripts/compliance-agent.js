#!/usr/bin/env node
/**
 * Seedance Compliance Agent v6.0-Peng
 * 智能合规闸门 — 在限制内把效果拉满 + LLM两层深度评估
 * 
 * 🆕 v6.0-Peng (2026-06-11): LLM两层深度评估接入
 *   - Layer 1: 语义完整性评估 (角色/动作/情绪/环境/运镜/光影 6维度)
 *   - Layer 2: 创意质量评估 (叙事张力/视觉冲击/情感共鸣/风格一致性)
...
class ComplianceAgent {
  ...
  /**
   * 主入口：合规检查与精炼
   * 🆕 v6.0-Peng: LLM两层深度评估优先，本地正则降级为fallback
   * @param {string} prompt - 原始提示词
   * @param {Object} shot - 镜头信息
   * @param {Array} refs - 参考图列表
   * @returns {Object} 合规结果
   */
  async check(prompt, shot = {}, refs = [], options = {}) {
    // 🆕 v6.0-Peng: 尝试LLM两层深度评估
    let llmEval = null;
    try {
      llmEval = await this._llmTwoLayerEval(prompt, shot, refs, options);
    } catch (err) {
      console.log(`   ⚠️ LLM评估失败: ${err.message?.substring(0, 60)}，降级本地正则`);
    }

    // 降级：本地正则检查
    return this._checkLocal(prompt, shot, refs, options, llmEval);
  }
   ...
   * Layer 2: 创意质量 — 4维度评估
   */
  async _llmTwoLayerEval(prompt, shot, refs, options) {
    if (options?.skipLLMEval) return null;

    const { LLMReasoningLayer } = require('./llm-reasoning-layer');
    const llmLayer = new LLMReasoningLayer();

    const systemPrompt = `You are a compliance evaluator for AI video prompts. Perform a TWO-LAYER evaluation and return ONLY valid JSON.

LAYER 1: Semantic Completeness (score 0-1.0)
Evaluate 6 dimensions:
- subject_anchor: Is the main character clearly identified?
- primary_action: Is there a clear core action?
- performance: Are emotional reactions described?
- environment: Is the spatial setting described?
- camera: Are camera movements specified?
- lighting: Is lighting/atmosphere described?

LAYER 2: Creative Quality (score 0-1.0)
Evaluate 4 dimensions:
- narrative_tension: Does the prompt create dramatic tension?
- visual_impact: Are the visuals striking and memorable?
- emotional_resonance: Does it evoke genuine emotion?
- style_consistency: Is the style coherent throughout?

OUTPUT JSON:
{"layer1":{"score":0.85,"dimensions":{"subject_anchor":0.9,"primary_action":0.8,"performance":0.7,"environment":0.9,"camera":0.8,"lighting":0.9},"missing":["performance detail"],"suggestions":["Add facial expression detail"]},"layer2":{"score":0.75,"dimensions":{"narrative_tension":0.7,"visual_impact":0.8,"emotional_resonance":0.7,"style_consistency":0.8},"strengths":["Strong visual imagery"],"weaknesses":["Emotional arc could be stronger"],"improvements":["Add emotional beat before action"]},"overall":{"score":0.80,"verdict":"PASS|WARN|BLOCK","summary":"One-line summary in Chinese"}}`;

    const userPrompt = `Evaluate this AI video prompt:

PROMPT: ${prompt}

SHOT TYPE: ${shot.type || 'normal'}
CHARACTERS: ${JSON.stringify((shot.characters || []).filter(Boolean))}
TENSION: ${shot.tension || 'N/A'}
REFS: ${refs.length} reference images

Provide the two-layer evaluation JSON.`;

    const result = await llmLayer.llmReason({
      stage: 'compliance-llm-eval',
  ...
  /**
   * 本地正则检查（原check逻辑 + LLM评估结果注入）
   */
  _checkLocal(prompt, shot, refs, options, llmEval) {
    ...
    return {
      ...
      warnings,
      openingCheck: openingCheck.summary,
      llmEval
    };
  }
