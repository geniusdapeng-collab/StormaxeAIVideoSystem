...
/**
 * Prompt Final Normalizer v1.0-Peng-fix
 *
 * 目标：
...
function buildOpeningTitleFields(shot, storyPlan, options = {}) {
  ...
  const dialogue = extractFieldValue(titlePrompt, 'Dialogue') || `SPEAKER: ${storyPlan?.title || 'Beast'} | TYPE: opening line | EMOTION: solemn | TEXT: "Welcome to planet Nirath." | LIP_SYNC: YES`;
  ...
  const scene = stripChinese(titlePrompt).slice(0, 1500) || 'Epic opening title scene on Nirath geological cliff landscape.';
 ...
 * 主入口：规范化单个 shot
 */
function normalizeShotPromptFields(shot, storyPlan, options = {}) {
 ...
 * 批量规范化
 */
function normalizeAllShots(shots, storyPlan, options = {}) {
  const results = [];
  const fs2 = require('fs');

  for (const shot of shots || []) {
    const prompt = normalizeShotPromptFields(shot, storyPlan, options);
