/**
 * smart-quality-calibration.js v3.0-Peng（专家方案整合版 + LLM）
 *
 * 🆕 v3.0-Peng (2026-06-11): LLM智能质量校准接入
 *   - LLM分析提示词并智能判断需要注入的质量填充内容
...
class SmartQualityCalibration {
  /**
   * 🆕 v3.0-Peng: LLM优先智能校准，失败降级本地规则
   */
  async calibrate(prompt, context = {}) {
    // 🆕 v3.0-Peng: 尝试LLM智能校准
    try {
      const llmResult = await this._calibrateWithLLM(prompt, context);
      if (llmResult) {
        console.log('   ✅ LLM智能质量校准完成');
        return llmResult;
      }
    } catch (err) {
      console.log(`   ⚠️ LLM校准失败: ${err.message?.substring(0, 60)}，降级本地规则`);
    }

    return this._calibrateLocal(prompt, context);
  }

  /**
   * 🆕 v3.0-Peng: LLM智能质量校准
   */
  async _calibrateWithLLM(prompt, context = {}) {
    const llm = new LLMReasoningLayer();
    const before = charCounter.count(String(prompt || ''));

    const result = await llm.llmReason({
      stage: 'smart-quality-calibration',
      systemPrompt: `You are a prompt engineering expert. Analyze the given AI video generation prompt and determine what quality enhancements it needs. Consider: render quality, worldview consistency, material detail, lighting atmosphere, camera language, performance detail, atmosphere, and negative constraints. Return a JSON object with: prompt (enhanced prompt string), before (original char count), after (new char count), strategy ("LLM_ENHANCED" or "PASS_THROUGH"), enhancements (array of what was added). Target length: ${CALIBRATION_CONFIG.TARGET_IDEAL} chars, hard limit: ${CALIBRATION_CONFIG.HARD_LIMIT}.`,
   ...
   * 本地规则校准（原calibrate逻辑）
   */
  _calibrateLocal(prompt, context = {}) {
    ...
    if (charCounter.count(output) < CALIBRATION_CONFIG.TARGET_MIN) {
      const secondPass = [lib.worldview, lib.characterSilhouette, lib.surfaceDetail].filter(Boolean);
