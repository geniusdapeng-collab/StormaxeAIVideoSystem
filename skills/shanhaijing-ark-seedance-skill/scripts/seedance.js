...
const TERMINAL_STATUSES = new Set([
  ...
  "expired",
  "cancelled",
  "completed" // 某些API版本使用completed而非succeeded
]);
...
async function handleRun(options) {
  ...
  let task = await waitForTask(client, taskId, options);
  // Fix-4: 如果任务失败且可重试，重新创建
  if (task === null) {
    console.error(`[Retry] Recreating task after failure...`);
    const retryResponse = await client.createTask(payload);
    const retryTaskId = assertTaskId(retryResponse);
    console.error(`Retry created: ${retryTaskId}`);
    task = await waitForTask(client, retryTaskId, options);
  }
  ...
  try {
    switch (platform) {
      case 'openclaw': {
        ...
        let originalConfig;
        try {
          originalConfig = JSON.parse(fsSync.readFileSync(configPath, 'utf8'));
        } catch (e) {
          console.error(`⚠️  OpenClaw 配置解析失败，使用空配置: ${e.message}`);
          originalConfig = {};
        }
      ...
      case 'claude-code': {
        ...
        if (fsSync.existsSync(configPath)) {
          ...
          try {
            ...
            console.error(`⚠️ Claude配置解析失败，使用空配置: ${e.message}`);
          }
          // 🛡️ 脱敏后备份，不泄漏已有密钥
          const sanitizedBackup = sanitizeApiKeys(originalConfig);
          fsSync.writeFileSync(backupPath, JSON.stringify(sanitizedBackup, null, 2));
...
async function buildPayload(options) {
  ...
  const hasMedia = 
    toArray(options.imageUrl).length > 0 ||
    toArray(options.imageFile).length > 0 ||
  ...
  if (is15pro && hasWebSearch) {
    throw new Error('⚠️  Seedance 1.5 pro 不支持联网搜索，请使用 doubao-seedance-2.0 或 doubao-seedance-2.0-fast');
  }
  
  if (is15pro && hasMedia) {
    throw new Error('⚠️  Seedance 1.5 pro 不支持图片/视频/音频参考，请使用 doubao-seedance-2.0 或 doubao-seedance-2.0-fast');
  }
...
async function waitForTask(client, taskId, options) {
  ...
  const MAX_POLL_COUNT = Math.ceil(timeoutMs / intervalMs) + 10; // 比理论次数多10次缓冲
  const MAX_INTERVAL_MS = 30000; // 最大间隔30秒
  ...
  while (pollCount < MAX_POLL_COUNT) {
    ...
    try {
      ...
      currentInterval = intervalMs; // 成功获取则重置间隔
      console.error(`Task ${taskId} status: ${status || "unknown"} (poll ${pollCount}/${MAX_POLL_COUNT})`);

      if (TERMINAL_STATUSES.has(status)) {
        if (status !== "succeeded" && status !== "completed") {
          const errorCode = task.error?.code || '';
          const RETRYABLE_TASK_ERRORS = ['rate_limit_exceeded', 'server_overload', 'internal_error', 'timeout'];
          // Fix-4: 可重试错误返回null让上层重新创建
          if (RETRYABLE_TASK_ERRORS.some(code => errorCode.includes(code))) {
            console.error(`Task ${taskId} failed with ${errorCode}, will allow retry recreation`);
            return null;
          }
          throw new Error(task.error ? `${task.error.code}: ${task.error.message}` : `Task ended with ${status}`);
        }
        return task;
      }

      if (Date.now() - startedAt > timeoutMs) {
        throw new Error(`Timed out waiting for task ${taskId} after ${Math.round(timeoutMs / 1000)} seconds (polls: ${pollCount}).`);
      }
    } catch (err) {
      // 限流429或超时时退避，其他错误直接抛
      if (err.message?.includes('429')) {
        currentInterval = Math.min(currentInterval * 2, MAX_INTERVAL_MS);
        console.error(`Task ${taskId} rate limited, backing off to ${currentInterval}ms`);
      } else if (err.message?.includes('timeout')) {
        currentInterval = Math.min(currentInterval * 1.5, MAX_INTERVAL_MS);
      } else if (RETRYABLE_ERRORS.some(code => err.message?.includes(code))) {
        currentInterval = Math.min(currentInterval * 1.5, MAX_INTERVAL_MS);
        console.error(`Task ${taskId} network error, backing off to ${currentInterval}ms`);
      } else {
        throw err;
      }
    }
  ...
  throw new Error(`Max poll count (${MAX_POLL_COUNT}) reached for task ${taskId}`);
}
...
async function downloadToFile(url, targetPath, timeoutMs = 300000) {
  ...
  try {
    // 检查是否有未完成的.partial文件（断点续传）
    if (fsSync.existsSync(partialPath)) {
      const stats = fsSync.statSync(partialPath);
      startByte = stats.size;
      console.error(`[download] Resuming from byte ${startByte}`);
    }
    ...
    // 校验 HTTP 状态码
    if (!response.ok && response.status !== 206) { // 206 Partial Content 是合法的
    ...
    // 🐛 流式下载：避免大视频一次性读入内存
    // 以append模式打开（续传）或create模式（新下载）
    fileStream = fsSync.createWriteStream(partialPath, { flags: startByte > 0 ? 'a' : 'w' });
    ...
    while (true) {
      ...
      if (Date.now() - lastProgressTime > 30000) {
        console.error(`[download] ${(totalBytes / 1024 / 1024).toFixed(1)}MB`);
    ...
    fileStream = null; // 标记已关闭
    
    // 下载完成才重命名
    fsSync.renameSync(partialPath, targetPath);
    ...
    // 如果是异常退出（未重命名为targetPath），保留.partial
    if (fsSync.existsSync(partialPath) && !fsSync.existsSync(targetPath)) {
      console.error(`[download] Kept partial file: ${partialPath}`);
    }
  }
}
...
const RETRY_BACKOFF_MS = [1000, 2000, 4000]; // 第1/2/3次重试等待时间
  ...
  for (let attempt = 0; attempt <= 3; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // 可重试错误且还有重试次数
        if (attempt < 3 && RETRYABLE_STATUSES.includes(response.status)) {
          console.error(`[requestJson] attempt ${attempt + 1} failed: ${errMsg}, retrying in ${RETRY_BACKOFF_MS[attempt]}ms`);
          await sleep(RETRY_BACKOFF_MS[attempt]);
          continue;
        }
        throw new Error(`Ark API request failed: ${errMsg} - ${sanitizeErrorText(text || errMsg)}`);
      }

      return data;
    } catch (err) {
      lastError = err;
      const isRetryable = RETRYABLE_ERRORS.some(code => err.message?.includes(code));
      if (attempt < 3 && isRetryable) {
        console.error(`[requestJson] attempt ${attempt + 1} network error: ${err.message}, retrying...`);
        await sleep(RETRY_BACKOFF_MS[attempt]);
        continue;
      }
      throw err;
...
async function readJson(filePath) {
  ...
  try {
    ...
    console.error(`[readJson] 解析失败: ${filePath} — ${e.message}`);
