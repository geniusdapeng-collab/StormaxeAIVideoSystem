...
function execCLI(args, { silent = false } = {}) {
  const result = spawnSync('node', [CLI_SCRIPT, ...args], {
    encoding: 'utf8',
    env: process.env,
    timeout: 300000, // 5分钟超时（匹配wrapper的超时策略）
    killSignal: 'SIGTERM', // 超时后先发送SIGTERM
    maxBuffer: 50 * 1024 * 1024 // 最大输出50MB（防止内存爆炸）
  });
  
  if (result.error && result.error.code === 'ETIMEDOUT') {
    throw new Error(`CLI command timed out after 300s: node ${CLI_SCRIPT} ${args.join(' ')}`);
  }
