 ...
 * 故事引擎脚本 - v5.9-Peng 兼容层
 * 
 * v5.9-Peng-fix: 通用优化修复（针对前10用例共性问题）
...
// v5.5-Peng-CinePrompt: 情绪-运镜映射表 - 张力值→电影级运镜词汇
...
// v5.5-Peng-CinePrompt: 舞蹈内容检测 - 检测大纲/标题是否含舞蹈关键词
...
// v5.5-Peng-CinePrompt: 应用情绪-运镜映射到shot
...
function parseUserOutline(outline, explicitCharacters) {
  ...
  // 3. 通用起承转合提取(与具体角色无关)
  if (outline) {
    // v5.7-Peng-fix: 修复正则匹配中文全角标点 + 支持承幕/转幕
  ...
  if (outline) {
    ...
    for (const segment of segments) {
      ...
      let actName = null;
      const actMarkers = {
        // v5.7-Peng-fix: 使用^锚点确保只匹配段落开头的幕标记
        '起': /^起[幕:]*/,
        '承': /^承|发展/,
        '转': /^转|转折/,
        '高潮': /^高潮/,
  ...
  if (outline) {
    ...
    for (const segment of segments) {
      ...
      // v5.7-Peng-fix: 第一个匹配的幕生效（段落开头的幕标记最准确）
      for (const [act, pattern] of Object.entries(actMarkers)) {
        if (pattern.test(trimmed)) { actName = act; break; }
      }
      // 提取该段落的动作关键词
...
function generateProgressiveDescription(baseDesc, shotIndexInAct, totalShotsInAct, shot, actName) {
  ...
  // v5.9-Peng-fix: 根据幕类型注入角色弧光关键词（转幕/高潮幕）
  const arcKeywords = {
  ...
  if (shotIndexInAct === 0) {
    ...
    // v5.9-Peng-fix: 第四个+镜头: 动态变化/高潮 + 弧光关键词
    const dynamicHints = [
...
function overrideWithUserOutline(plan, outlineData, style = '') {
  ...
  // 2. 替换所有 shot 中的角色引用  // v5.5-Peng-fix: 角色注入过度修复 - 每镜头只注入该场景需要的角色
  ...
  for (const shot of plan.shots || []) {
    if (shot.characters) {
      ...
      // v5.5-Peng-fix: 角色注入过度修复 - 用智能推断替换全部注入
    ...
    // v5.7-Peng-fix: 同幕description去重 + 场景/动作关键词注入
    if (shot.act && acts[shot.act]) {
      ...
      if (!hasUniqueDesc || !shot.description || shot.description === outlineDesc) {
        // 仅在原始全部相同或为空时，做递进覆盖
        // v5.9-Peng-fix: 传入shot.act以注入角色弧光关键词
    ...
    // v5.5-Peng-CinePrompt: 情绪-运镜映射(舞蹈/非舞蹈通用)
  ...
  plan.characters = chars.map((name, i) => {
    ...
    if (i === 0) role = 'protagonist';
      ...
      // v5.7-Peng-fix: 根据videoType判断第二角色关系
  ...
  // v5.5-Peng-fix: 修正幕-type一致性 — 确保每幕的shots type与该幕功能匹配
  for (const shot of plan.shots || []) {
    const act = shot.act || '';
  ...
  // v5.5-Peng-fix: 合幕缺失修复 - 当 StoryForge Pro 未生成合幕时,从用户大纲主动注入
  ...
  // v5.7-Peng-fix: 时长精确控制 - 不仅补偿不足，还缩减超标
  ...
  if (actualDuration < targetDuration) {
    ...
    // v5.7-Peng-fix: 如果仍然超标，删除次要镜头（降低阈值到目标时长）
    ...
    // v5.7-Peng-fix: 最终检查，如果仍然超标，进一步缩减每个镜头（最低2s）
  ...
  // v5.7-Peng-fix: 更新 totalShots 和 totalDuration
  plan.totalShots = (plan.shots || []).length;
...
async function runPlan(args) {
  ...
  const orchestrator = path.join(workspace, 'skills/storyforge-pro/orchestrator.js');
  ...
  // 读取 StoryForge Pro 生成的 01-story-plan.json
  const sfpPlanPath = path.join(tmpDir, '01-story-plan.json');
  if (!fs.existsSync(sfpPlanPath)) {
    throw new Error('StoryForge Pro 未生成 01-story-plan.json');
  }

  const plan = JSON.parse(fs.readFileSync(sfpPlanPath, 'utf8'));
  
  // v5.7-Peng-fix: 确保targetDuration从用户参数传递
