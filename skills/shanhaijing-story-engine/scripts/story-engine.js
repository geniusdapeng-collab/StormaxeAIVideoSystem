 ...
 * 内部调用: skills/shanhaijing-storyforge-pro/orchestrator.js create
 *
 * 用法:
...
function applyEmotionCameraMap(shot, isDance = false, style = '') {
  ...
  if (isDance) {
    ...
    const existingStyle = styleMarker && shot.camera?.includes(styleMarker.split(',')[0]) ? `,${styleMarker.split(',')[0]}风格` : '';
...
function overrideWithUserOutline(plan, outlineData, style = '') {
  ...
  function inferCharactersForShot(shot, allChars, outlineData) {
    ...
    // 🟢 v6.0-Peng-fix: 主角覆盖率强制校验
    // 主角(C01)必须在所有镜头中出现，不可遗漏
    ...
    // 🟢 v6.0-Peng-fix: 战斗/动作类型强制双角色在场
    const isActionScene = /战斗|挥|舞|对决|冲击|能量|觉醒|爆发|干戚|盾牌|斧|武器|击打|碰撞/.test(desc);
    if (isActionScene && allChars.length >= 2) {
      // 战斗场景必须主角+神兽都在场，形成观察者-表演者张力
      needed.add(allChars[0]); // 主角
      needed.add(allChars[1]); // 神兽
    }
    ...
    // 🟢 v6.0-Peng-fix: 高潮/转幕强制所有角色在场
    if ((act === '高潮' || act === '转') && allChars.length >= 2) {
      for (const char of allChars) needed.add(char);
    }
...
async function runPlan(args) {
  ...
  const orchestrator = path.join(__dirname, '../../shanhaijing-storyforge-pro/orchestrator.js');
...
const FORESHADOWING_TYPES = {
  PROP: 'prop',           // 道具型伏笔（半张照片、笔记本、羽毛笔）
  DIALOGUE: 'dialogue',   // 对白型伏笔（帝江哼唱的旋律）
  ACTION: 'action',       // 动作型伏笔（触摸背纹）
  VISUAL: 'visual',       // 视觉型伏笔（背纹发光如地图）
  MYTH: 'myth'            // 神话型伏笔（《山海经》未出现的异兽）
};

/**
 * 伏笔回收方式
 */
const RESOLUTION_METHODS = ['callback', 'reveal', 'twist', 'payoff'];

/**
 * 三幕信息释放策略（1/8 → 2/8 → 5/8）
 */
const INFO_RELEASE_STRATEGY = {
  act1: { maxNewInfoRatio: 0.125, focus: 'character_relationship', infoType: 'surface' },
  act2: { maxNewInfoRatio: 0.25, focus: 'world_construction', infoType: 'deepening' },
  act3: { maxNewInfoRatio: 0.625, focus: 'myth_revelation', infoType: 'core' }
};

class ForeshadowingEngine {
  constructor() {
    this.foreshadowings = [];  // 所有伏笔条目
    this.episodeRegistry = new Map(); // episodeId -> 伏笔ID列表
    this.beastRegistry = new Map();   // beastId -> 伏笔ID列表
    this.nextId = 1;
  }

  /**
   * 设置伏笔
   * @param {string} episodeId - 设置伏笔的集ID (如 'E01')
   * @param {string} type - 伏笔类型 (prop/dialogue/action/visual/myth)
   * @param {string} content - 伏笔内容描述
   * @param {Object} options - 选项
   * @param {string} [options.targetEpisode] - 目标回收集 (如 'E05')
   * @param {string} [options.targetBeast] - 关联异兽ID
   * @param {string} [options.sceneContext] - 场景上下文
   * @param {number} [options.expiryEpisodes=3] - 过期集数（超过未回收则过期）
   * @returns {Object} 创建的伏笔对象
   */
  plant(episodeId, type, content, options = {}) {
    const id = `FS_${String(this.nextId++).padStart(3, '0')}`;
    const foreshadowing = {
      id,
      episodeId,
      type,
      ...
      targetEpisode: options.targetEpisode || null,
      targetBeast: options.targetBeast || null,
      sceneContext: options.sceneContext || '',
      expiryEpisodes: options.expiryEpisodes || 3,
    ...
    // 注册到集索引
    if (!this.episodeRegistry.has(episodeId)) {
      this.episodeRegistry.set(episodeId, []);
    }
    this.episodeRegistry.get(episodeId).push(id);

    // 注册到异兽索引
    if (options.targetBeast) {
      if (!this.beastRegistry.has(options.targetBeast)) {
        this.beastRegistry.set(options.targetBeast, []);
      }
      this.beastRegistry.get(options.targetBeast).push(id);
    }

    console.log(`🔮 伏笔已设置 [${id}] ${type}: "${content.substring(0, 30)}..." @ ${episodeId}`);
    return foreshadowing;
  }

  /**
   * 回收伏笔
   * @param {string} foreshadowingId - 伏笔ID
   * @param {string} episodeId - 回收集ID
   * @param {string} method - 回收方式: callback | reveal | twist | payoff
   * @param {Object} options - 选项
   * @param {string} [options.resolutionContext] - 回收上下文描述
   * @returns {Object|null} 更新后的伏笔对象
   */
  resolve(foreshadowingId, episodeId, method, options = {}) {
    const fs = this.foreshadowings.find(f => f.id === foreshadowingId);
    if (!fs) {
      console.error(`❌ 伏笔不存在: ${foreshadowingId}`);
      return null;
    }
    if (fs.status !== FORESHADOWING_STATUS.PLANTED) {
      console.warn(`⚠️ 伏笔 ${foreshadowingId} 状态为 ${fs.status}，不可回收`);
      return fs;
    }

    if (!RESOLUTION_METHODS.includes(method)) {
      console.error(`❌ 无效回收方式: ${method}，可用: ${RESOLUTION_METHODS.join(', ')}`);
      return null;
    }

    fs.status = FORESHADOWING_STATUS.RESOLVED;
    fs.resolvedAt = new Date().toISOString();
    fs.resolutionEpisode = episodeId;
    fs.resolutionMethod = method;
    fs.resolutionContext = options.resolutionContext || '';

    console.log(`✅ 伏笔已回收 [${foreshadowingId}] via ${method} @ ${episodeId}`);
    return fs;
  }
   ...
   * @returns {Object|null} 更新后的伏笔对象
   */
  abandon(foreshadowingId, reason = '') {
    const fs = this.foreshadowings.find(f => f.id === foreshadowingId);
    if (!fs) {
      console.error(`❌ 伏笔不存在: ${foreshadowingId}`);
      return null;
    ...
    if (fs.status !== FORESHADOWING_STATUS.PLANTED) {
      console.warn(`⚠️ 伏笔 ${foreshadowingId} 状态为 ${fs.status}，不可废弃`);
      return fs;
    }

    fs.status = FORESHADOWING_STATUS.ABANDONED;
    fs.abandonedAt = new Date().toISOString();
    fs.abandonReason = reason;

    console.log(`🗑️ 伏笔已废弃 [${foreshadowingId}] 原因: ${reason || '未指定'}`);
    return fs;
  }
   ...
   * @returns {Array} 过期的伏笔列表
   */
  checkExpiry(currentEpisodeId) {
    const currentNum = parseInt(currentEpisodeId.replace('E', ''));
    const expired = [];

    for (const fs of this.foreshadowings) {
      if (fs.status !== FORESHADOWING_STATUS.PLANTED) continue;

      const plantedNum = parseInt(fs.episodeId.replace('E', ''));
      const age = currentNum - plantedNum;

      if (age > fs.expiryEpisodes) {
        fs.status = FORESHADOWING_STATUS.EXPIRED;
        fs.expiredAt = new Date().toISOString();
        expired.push(fs);
        console.log(`⏰ 伏笔过期 [${fs.id}] 设置于${fs.episodeId}，已${age}集未回收`);
      }
    }
   ...
   * @returns {Array} 伏笔列表
   */
  getByEpisode(episodeId, status = null) {
    const ids = this.episodeRegistry.get(episodeId) || [];
    const fss = ids.map(id => this.foreshadowings.find(f => f.id === id)).filter(Boolean);
    return status ? fss.filter(f => f.status === status) : fss;
  }
   ...
   * @returns {Array} 伏笔列表
   */
  getByBeast(beastId, status = null) {
    const ids = this.beastRegistry.get(beastId) || [];
    const fss = ids.map(id => this.foreshadowings.find(f => f.id === id)).filter(Boolean);
    return status ? fss.filter(f => f.status === status) : fss;
  }
  ...
  /**
   * 获取所有未回收伏笔
     * @returns {Array} 未回收伏笔列表
   */
  getUnresolved() {
    return this.foreshadowings.filter(f => f.status === FORESHADOWING_STATUS.PLANTED);
  }
  ...
  getStats() {
    ...
    const planted = this.foreshadowings.filter(f => f.status === FORESHADOWING_STATUS.PLANTED).length;
    const resolved = this.foreshadowings.filter(f => f.status === FORESHADOWING_STATUS.RESOLVED).length;
    const abandoned = this.foreshadowings.filter(f => f.status === FORESHADOWING_STATUS.ABANDONED).length;
    const expired = this.foreshadowings.filter(f => f.status === FORESHADOWING_STATUS.EXPIRED).length;
    ...
    return {
      ...
      abandoned,
      expired,
      recoveryRate: total > 0 ? Math.round((resolved / total) * 1000) / 10 : 0,
      expiryRate: total > 0 ? Math.round((expired / total) * 1000) / 10 : 0
    };
  }
  ...
  export() {
    return {
      ...
      stats: this.getStats(),
      episodeRegistry: Object.fromEntries(this.episodeRegistry),
      beastRegistry: Object.fromEntries(this.beastRegistry),
   ...
   */
  import(data) {
    if (data.foreshadowings) {
      this.foreshadowings = data.foreshadowings;
    }
    if (data.episodeRegistry) {
      this.episodeRegistry = new Map(Object.entries(data.episodeRegistry));
    }
    if (data.beastRegistry) {
      this.beastRegistry = new Map(Object.entries(data.beastRegistry));
    }
    // 重新计算nextId
    const maxId = this.foreshadowings.reduce((max, f) => {
      const num = parseInt(f.id.replace('FS_', ''));
      return Math.max(max, num);
    }, 0);
    this.nextId = maxId + 1;
    console.log(`📥 伏笔引擎已导入: ${this.foreshadowings.length}条伏笔`);
  }

  /**
   * 计算三幕信息释放配比
   * @param {string} act - 幕标识: act1 | act2 | act3
   * @param {number} currentInfoCount - 当前已释放信息数
   * @param {number} totalPlannedInfo - 计划总信息数
   * @returns {Object} 释放建议
   */
  calculateInfoRelease(act, currentInfoCount, totalPlannedInfo) {
    const strategy = INFO_RELEASE_STRATEGY[act];
    if (!strategy) {
      return { error: `未知幕: ${act}` };
    }

    const targetRatio = strategy.maxNewInfoRatio;
    const targetCount = Math.round(totalPlannedInfo * targetRatio);
    const remaining = targetCount - currentInfoCount;
    ...
    return {
      ...
      infoType: strategy.infoType,
      suggestion: canRelease
        ? `可释放${remaining}条新信息（${strategy.infoType}类型，聚焦${strategy.focus}）`
        : `${act}信息量已达上限(${targetRatio * 100}%)，建议复用旧信息或留到下一幕`
    };
  }
}
